...
import {
  ...
  LogOut, Hospital, UserCog, X, CreditCard, Banknote,
} from 'lucide-react';
...
const navItems: NavItem[] = [
  ...
  { to: '/billing',      label: 'Billing',      icon: <Banknote size={18} />,        roles: ['ADMIN'] },
...
export default function Sidebar({ onClose }: Props) {
  ...
  return (
    <aside className="flex h-full w-64 flex-col bg-primary-900 text-white">
      ...
      {/* Logo row */}
      <div className="flex items-center gap-3 px-5 py-4 border-b border-primary-800">
        <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-primary-500 shrink-0">
          <Hospital size={20} />
        </div>
        ...
        {onClose && (
          <button
            onClick={onClose}
            className="md:hidden p-1 rounded-lg text-primary-300 hover:text-white hover:bg-primary-800 transition-colors"
